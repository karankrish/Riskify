...
def combine_company_balance_sheets():
    folder_path = input("📁 Enter the main folder path (containing sector folders): ").strip()
            ...
            for filename in os.listdir(sector_path):
                if filename.lower().endswith(".csv"):
                    ...
                    try:
                        ...
                        # 🧱 Add Sector and Company columns
                        df.insert(loc=0, column='Industry', value=sector)
                        df.insert(loc=2, column='Company', value=company_name)
                        ...
                        keep_cols = ['Industry', 'Financial_Metric', 'Company']
                        ...
                        fixed_cols = ['Industry', 'Financial_Metric', 'Company']
                        ...
                        print(f"✅ Processed: {sector}/{company_name}")

                    except Exception as e:
                        print(f"⚠ Error processing {sector}/{filename}: {e}")

    if not combined_data:
        print("❌ No valid CSV files found in any sector folder.")
        return
    ...
    fixed_columns = ['Industry', 'Financial_Metric', 'Company']
    ...
    # Handle sparse rows: polynomial if enough points, else linear
    # Handle sparse rows: polynomial if enough points, else linear
    def interpolate_row(row):
        if row.notna().sum() >= 3:
            return row.interpolate(method='polynomial', order=2, limit_direction='both').bfill().ffill()
        else:
            return row.interpolate(method='linear', limit_direction='both').bfill().ffill()


    year_data_interp = year_data.apply(interpolate_row, axis=1)
    year_data_interp.columns = year_columns
    final_df[year_columns] = year_data_interp

    # 📊 Add summary (Average/Median) rows
    main_categories = ['Total Assets', 'Total Liabilities', 'Equity Capital', 'Reserves', 'Borrowings',
                       'Other Liabilities', 'Fixed Assets', 'CWIP', 'Investments', 'Other Assets']
